...
def parse_arguments():
    ...
    Devuelve:
        filepath (str): La ruta del archivo proporcionada por el usuario.
    """
    unixOptions = "f:"
    gnuOptions = ["file="]
...
def preprocess_data (df, threshold=0.5):
    """
    Preprocesa el DF elimidando las filas en las que la mayoría de las columnas contienen el caracter '*'. 

    Args:
        df (pd.DataFrame): El DF de entrada. 
    Devuelve: 
    ...
    new_col_name = ['PERTENENCIA_ESTABLECIMIENTO_SALUD', 'SEXO', 'GRUPO_EDAD', 'ETNIA',
    ...
    column_mapping = dict(zip(old_column_names, new_col_name))
